...
//export let controls = [38, 37, 40, 39];
...
//export let vibrate = false;
export let offsetVibX = 0;
export let offsetVibY = 0;
export function vibrate(time, amt) {
  offsetVibX = Math.round(Math.random() * amt);
  offsetVibY = Math.round(Math.random() * amt);
  if (time > 1 && amt > 1)
    setTimeout(() => {
      vibrate(time - 10, amt - 1);
    }, 10);
}

function preupdateGameArea() {
  updateGameArea();
}
...
export let game = {
  ...
  start: function () {
    ...
    this.interval = setInterval(preupdateGameArea, Math.round(1000 / 60));
  },
  clear: function () {
...
export function rectOutline(x, y, w, h, lnWidth) {
  var lnW = -lnWidth;
  //Top
  rect(x, Math.floor(y), w, lnWidth);
  //Bottom
  rect(x, Math.ceil(y + h), w, lnW);
  //Left
  rect(x + lnWidth, y, lnW, h);
  //Right
  rect(x + w, y, lnW, h + lnW + 1);
}
...
export function inArea(X, Y, x, y, w, h) {
  return X > x - 1 && Y > y - 1 && X < x + w && Y < y + h;
}
...
/**
 * @param {Number} x1 - The first points x pos
 * @param {Number} y1 - the first points y pos
 * @param {Number} x2 - The second points x pos
 * @param {Number} y2 - the second points y pos
 */
...
export function DownloadWorld() {
  let downloadData = mapWidth + " " + mapHeight + "\n";
  ...
  for (let i = 0; i < mapWidth; i++) {
    for (let j = 0; j < mapHeight; j++) {
      downloadData = downloadData + " " + mapData[i][j];
      //map[i][j] = Math.round(Math.random(2)) + 1;
    }
  }
  downloadData = downloadData + "\n";
  ...
  for (let i = 0; i < boxes.length; i++) {
    downloadData = downloadData + " " + boxes[i].x + "-" + boxes[i].y;
    //map[i][j] = Math.round(Math.random(2)) + 1;
  }

  download(downloadData, "world.txt", "test");
}
//Generic Load world
export function loadWorld(worldName) {
  ...
  (async () => {
    ...
    var blockData = spl[2].split(" ");
    let cratePostions = spl[3].split(" ");

    // console.log(spl[1].split(" ");
    ...
    for (var i = 0; i < mapWidth; i++) {
      for (var j = 0; j < mapHeight; j++) {
        i1++;
        //console.log(i1 + " " + blocks[i1]);
        //map[i][j] = 0;
        map[i][j] = parseInt(blocks[i1]);
        mapData[i][j] = blockData[i1];

        //map[i][j] = Math.round(Math.random(2)) + 1;
      }
    }

    for (let i = 0; i < cratePostions.length; i++) {
      let c = cratePostions[i].split("-");
      boxes.push(new pushableBox(parseInt(c[0]), parseInt(c[1])));
    }
